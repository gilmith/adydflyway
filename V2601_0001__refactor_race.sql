...
declare 
	...
	v_exe text;
begin
	select count(1) into exists_column from information_schema."columns" where table_catalog = 'adyd' and table_name = 'race'
	and column_name = 'minimum_strength';
	if(exists_column = 1) then
		v_exe = 'alter table race drop column minimum_strength';
	...
	if(exists_column = 1) then
		v_exe = 'alter table race drop column minimum_dexterity';
	...
	if(exists_column = 1) then
		v_exe = 'alter table race drop column minimum_inteligence';
	...
	if(exists_column = 1) then
		v_exe = 'alter table race drop column minimum_wisdom';
	...
	if(exists_column = 1) then
		v_exe = 'alter table race drop column minimum_charisma';
	...
	if(exists_column = 1) then
		v_exe = 'alter table race drop column minimum_constitution';
		raise notice 'dropping minimum_constitution';
		execute v_exe;
	end if;

	alter table race add if not exists description text;
